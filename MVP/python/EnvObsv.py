...
def getAirTopTempObsv(test=False):
    ...
    return makeEnvJson(activity_type, 'Air', 'Left_Side', 'Temperature', '{:3.1f}'.format(temp), 'SI7021', status_qualifier, comment=cmt)
...
def getAirTopHumidityObsv(test=False):
    ...
    if test:
        status_qualifier='Test'
    return makeEnvJson(activity_type, 'Air', 'Left_Side', 'Humidity', '{:3.1f}'.format(humid), 'SI7021', status_qualifier, comment=cmt)
...
def testEnv():    
    # Get std MVP JSON sensor reading
    makeEnvObsv()
    # Get test JSON sensor readings
    # Get test SQL readings
